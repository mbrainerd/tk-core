...
def parse_arguments_populate_environ():
    """
    process the arguments
    """
    parser = argparse.ArgumentParser(description="""Sample usage :
    shotgun_publish --path SGTK_PUBLISH_PRELOAD_PATH""")

    parser.add_argument("--p", '--preload_path', type=str,
                        dest='preload_path', default=None,
                        help='Preload path to use for file collection.')

    args = parser.parse_args()

    if args.preload_path:
        os.environ["SGTK_PUBLISH_PRELOAD_PATH"] = args.preload_path


if __name__ == "__main__":
    parse_arguments_populate_environ()
    main()
