...
def build_path(*paths):
    ...
    >>> self.build_path('$ABC_PREFIX/python/$PYTHON_VERSION', '$PYTHONPATH')
    ...
    if unresolved or non_existent:
        if unresolved:
            log.debug("expandPaths ignored paths with undefined variables:\n- %s", "\n- ".join(unresolved))
        if non_existent:
            log.debug("expandPaths ignored paths that don't exist:\n- %s", "\n- ".join(non_existent))
...
def combine_paths(*paths):
    ...
    >>> paths = combinePaths(['shot', 'seq'], ['devl', 'shared'], ['one', 'two'])
    >>> for path in paths:
    ...   print path
    shot/devl/one
    shot/devl/two
    ...
    The catenation behaviour is the same as :func:`os.path.join`.
    """
