...
def _resolve_template_r(template_paths, template_strings, template_aliases, template_name,
        template_definition, template_type, template_chain=None):
    """
    ...
    if template_key in visited_templates:
        raise TankError("A cyclic %s template was found - '%s' references itself (%s)"
                        % (template_type, template_name, " -> ".join([name for name,
                        _ in visited_templates[visited_templates.index(template_key):]] + [template_name])))
    ...
    for part in template_str_parts:
        ...
        for ref_part in ref_parts[1:]:
            ...
            if not match:
                raise TankError("Failed to parse template reference '@%s'" % ref_part)

            ref_name = match.group(1)
            ref_extra = match.group(2)

            # find a template that matches the reference name
            if ref_name in template_paths:
                ref_definition = template_paths[ref_name]
                ref_type = "path"
            elif ref_name in template_strings:
                ref_definition = template_strings[ref_name]
                ref_type = "string"
            elif ref_name in template_aliases:
                ref_definition = template_aliases[ref_name]
                ref_type = "alias"
            else:
                raise TankError("Failed to resolve template reference '@%s' defined by "
                                "the %s template '%s'" % (ref_name, template_type, template_name))

            # resolve the referenced template:
            resolved_ref = _resolve_template_r(template_paths,
                                               template_strings,
                                               template_aliases,
                                               ref_name,
                                               ref_definition,
            ...
            # Add back any remaining parts
            resolved_ref_parts.append(resolved_ref + ref_extra)
